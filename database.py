...
#conn.execute('CREATE TABLE if not exists article (article_id INTEGER PRIMARY KEY NOT NULL, article_title TEXT, author INTEGER NOT NULL, content TEXT NOT NULL, isActiveArticle TEXT, date_created INTEGER, date_modified INTEGER, FOREIGN KEY (author) REFERENCES users(user_name))')
...
conn.close()
#CREATE TABLE if not exists comments (comment_id INTEGER PRIMARY KEY, comment TEXT, article_id INTEGER, tag_id INTEGER, timestamp INTEGER, FOREIGN KEY (article_id) REFERENCES article(article_id), FOREIGN KEY (tag_id) REFERENCES tags(tag_ID))
